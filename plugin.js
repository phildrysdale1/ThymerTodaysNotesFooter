...
(function pluginSettingsRuntime(g) {
    ...
    const MOBILE_INTERACT_THROTTLE_AT_KEY =
        "__thymerExtMobileInteractThrottleAt";
            ...
            () => {
                try {
                    if (document.visibilityState === "hidden") {
                        ...
                        if (away >= MOBILE_RESUME_AWAY_MS)
                            bumpMobileLoadGrace(MOBILE_RESUME_GRACE_MS);
                    }
                } catch (_) {}
            },
            { passive: true },
    ...
    function pauseHeavyWorkQueue(ms) {
        if (!preferDeferredHeavyWork()) return;
        const until =
            Date.now() + (ms > 0 ? ms : MOBILE_HEAVY_PAUSE_ON_INTERACT_MS);
        try {
            if (
                !g[HEAVY_QUEUE_PAUSED_UNTIL_KEY] ||
    ...
    function touchGetAllSanityFromCount(len) {
        ...
        if (!h[GETALL_COLLECTIONS_SANITY])
            h[GETALL_COLLECTIONS_SANITY] = { nLast: 0, tLast: 0 };
    ...
    function chainPluginBackendEnsure(data, work) {
        ...
        try {
            if (!root[PB_ENSURE_GLOBAL_P])
                root[PB_ENSURE_GLOBAL_P] = Promise.resolve();
        } catch (_) {
            return Promise.resolve().then(work);
        }
        root[PB_ENSURE_GLOBAL_P] = root[PB_ENSURE_GLOBAL_P].catch(
            () => {},
        ).then(work);
    ...
    const PLUGIN_SETTINGS_SHAPE = {
        ...
        description:
            "Workspace storage for plugins: Use the Plugin column to filter by plugin.",
    ...
    function mergeViewsArray(baseViews, desiredViews) {
        const desired = Array.isArray(desiredViews)
            ? desiredViews.map((v) => cloneFieldDef(v))
            : [];
        const cur = Array.isArray(baseViews)
            ? baseViews.map((v) => cloneFieldDef(v))
            : [];
        if (cur.length === 0) {
    ...
    function colorForSlug(slug) {
        ...
        for (let i = 0; i < s.length; i++)
            h = (h + s.charCodeAt(i) * (i + 1)) % colors.length;
        return colors[h];
    }
    ...
    async function reconcilePluginFieldAsChoice(coll, curFields, desired) {
        const desiredPlugin = desired.fields.find(
            (f) => f && f.id === FIELD_PLUGIN,
        );
    ...
    async function registerPluginSlug(data, { slug, label } = {}) {
        ...
        if (
            !coll ||
            typeof coll.getConfiguration !== "function" ||
            typeof coll.saveConfiguration !== "function"
        )
            return;
        ...
        try {
            ...
            const normalized = prevChoices
                .map((c) => normalizeChoiceOption(c))
                .filter(Boolean);
            ...
            if (JSON.stringify(prev) !== JSON.stringify(next)) {
                fields[idx] = next;
                const ok = await coll.saveConfiguration(
                    withUnlockedManaged({ ...base, fields }),
                );
                if (ok === false)
                    console.warn(
                        "[ThymerPluginSettings] registerPluginSlug: saveConfiguration returned false",
                    );
    ...
    async function upgradePluginSettingsSchema(data, collOpt) {
        ...
        try {
            ...
            const curFields = Array.isArray(base.fields)
                ? base.fields.map((f) => cloneFieldDef(f))
                : [];
            const curIds = new Set(
                curFields.map((f) => (f && f.id ? f.id : null)).filter(Boolean),
            );
            let changed = false;
            for (const f of desired.fields) {
                ...
                if (
                    f.id === FIELD_PLUGIN &&
                    findPluginColumnFieldIndex(curFields) >= 0
                )
                    continue;
            ...
            const rec = await reconcilePluginFieldAsChoice(
                coll,
                curFields,
                desired,
            );
            ...
            if (JSON.stringify(curPages) !== JSON.stringify(mergedPages))
                changed = true;
            if ((base.description || "") !== desired.description)
                changed = true;
            if ((base.item_name || "") !== (desired.item_name || ""))
                changed = true;
            ...
            if (changed) {
                const merged = withUnlockedManaged({
                    ...
                    fields: finalFields,
                    page_field_ids: mergedPages.length
                        ? mergedPages
                        : wantPages,
                    ...
                    views: finalViews,
                    sidebar_record_sort_field_id:
                        desired.sidebar_record_sort_field_id ||
                        base.sidebar_record_sort_field_id,
                    sidebar_record_sort_dir:
                        desired.sidebar_record_sort_dir ||
                        base.sidebar_record_sort_dir,
                });
                const ok = await coll.saveConfiguration(merged);
                if (ok === false)
                    console.warn(
                        "[ThymerPluginSettings] saveConfiguration returned false (schema not applied?)",
                    );
                else {
                    try {
    ...
    async function rewritePluginChoiceCells(coll) {
        ...
        for (const r of records) {
            ...
            if (!slug)
                slug = rowField(r, pluginColumnPropId(coll, FIELD_PLUGIN));
            if (!slug) continue;
            ...
            if (!pidNow) {
                ...
                if (!kind) {
                    try {
                        ...
                        if (raw && String(raw).includes('"storageMode"'))
                            legacyVault = true;
                    } catch (_) {}
                }
    ...
    function pluginChoiceSetName(coll, slug) {
        ...
        if (!s || !coll || typeof coll.getConfiguration !== "function")
            return s;
        try {
            const f = getPluginFieldDef(coll);
            if (!f || f.type !== "choice" || !Array.isArray(f.choices))
                return s;
            const opt = f.choices.find(
                (c) => c && String(c.id || "").trim() === s,
            );
            if (opt && opt.label != null && String(opt.label).trim() !== "")
    ...
    function setRowField(r, id, value, coll = null) {
        ...
        try {
            ...
            if (isPluginChoiceCol && typeof p.setChoice === "function") {
                ...
                console.warn(
                    "[ThymerPluginSettings] setChoice: no option matched field",
                    id,
                    "slug",
                    s,
                    "tried",
    ...
    const LS_RECENT_CREATE_ATTEMPT_BASE =
        "thymerext_plugin_backend_recent_create_attempt_v1";

    function workspaceSlugFromData(data) {
        try {
            const u =
                data && typeof data.getActiveUsers === "function"
                    ? data.getActiveUsers()
                    : null;
    ...
    async function acquirePluginBackendCreationLease(maxWaitMs, data) {
        ...
        if (!ls) {
            ...
            if (DEBUG_COLLECTIONS) {
                dlogPathB("lease_denied_no_localstorage_no_locks", {
                    ws: workspaceSlugFromData(data),
                });
            }
        ...
        const deadline =
            Date.now() + (Number(maxWaitMs) > 0 ? maxWaitMs : 12000);
        let acquired = false;
        ...
        while (Date.now() < deadline) {
            try {
                ...
                if (busy) {
                    sawContention = true;
                    await new Promise((r) =>
                        setTimeout(r, 40 + Math.floor(Math.random() * 70)),
                    );
                    continue;
                }
        ...
        if (!acquired) {
            if (DEBUG_COLLECTIONS)
                dlogPathB("lease_timeout_abort_create", {
                    sawContention,
                    leaseKey,
                });
            return { denied: true, release() {} };
    ...
    function noteRecentPluginBackendCreate(data) {
        ...
        try {
            ls.setItem(
                scopedPbLsKey(LS_RECENT_CREATE_BASE, data),
                String(Date.now()),
            );
        } catch (_) {}
    }
    ...
    function noteRecentPluginBackendCreateAttempt(data) {
        ...
        try {
            ls.setItem(
                scopedPbLsKey(LS_RECENT_CREATE_ATTEMPT_BASE, data),
                String(Date.now()),
            );
        } catch (_) {}
    }
    ...
    function pathBCollectionScore(c) {
        ...
        try {
            ...
            if (
                nm &&
                nm.includes("plugin") &&
                (nm.includes("backend") || nm.includes("setting"))
            )
                s += 1;
            return s;
        } catch (_) {
        ...
        try {
            const pick = (allIn) => {
                ...
                return (
                    ...
                    list.find(
                        (c) => collectionDisplayName(c) === COL_NAME_LEGACY,
                    ) ||
                    list.find(
                        (c) => collectionBackendConfiguredTitle(c) === COL_NAME,
                    ) ||
                    list.find(
                        (c) =>
                            collectionBackendConfiguredTitle(c) ===
                            COL_NAME_LEGACY,
                    ) ||
                    null
                );
    ...
    function markWorkspacePluginBackendEnsureDone(data) {
        try {
            ...
            const h = getSharedDeduplicationWindow();
            if (!h[WS_ENSURE_OK_MAP] || typeof h[WS_ENSURE_OK_MAP] !== "object")
                h[WS_ENSURE_OK_MAP] = Object.create(null);
    ...
    function pathBWindowSnapshot() {
        ...
        try {
            ...
            snap.hostIsTop =
                host === (typeof window !== "undefined" ? window.top : null);
            snap.hostIsSelf =
    ...
    function queueDataCreateOnSharedWindow(factory) {
        ...
        try {
            if (
                !host[SERIAL_DATA_CREATE_P] ||
                typeof host[SERIAL_DATA_CREATE_P].then !== "function"
            ) {
                host[SERIAL_DATA_CREATE_P] = Promise.resolve();
            }
            const out = (host[SERIAL_DATA_CREATE_P] = host[
                SERIAL_DATA_CREATE_P
            ].catch(() => {}).then(factory));
            if (DEBUG_COLLECTIONS)
                dlogPathB("queueDataCreate_chained", {
                    gHasCreateQ: !!host[SERIAL_DATA_CREATE_P],
                });
            return out;
        } catch (e) {
            if (DEBUG_COLLECTIONS)
                dlogPathB("queueDataCreate_fallback", {
            ...
            try {
                if (data && data.getAllCollections) {
                    ...
                    const collNames = list.map((c) => {
                        try {
                            return (
                                String(collectionDisplayName(c) || "").trim() ||
                                "(no-name)"
                            );
                        } catch (__) {
                            return "(err)";
                    ...
                    dlogPathB("ensureBody_collections", {
                        count: (collNames && collNames.length) || 0,
                        names: (collNames || []).slice(0, 40),
                    });
                    if (data && data.getAllCollections)
                        touchGetAllSanityFromCount(
                            (collNames && collNames.length) || 0,
                        );
                    ...
                    if (dupExact.length > 1) {
                        dlogPathB(
                            "duplicate_plugin_backend_named_collections",
                            {
                ...
                dlogPathB("ensureBody_getAll_failed", {
                    err: String((e && e.message) || e),
                });
            }
        }
        try {
            ...
            try {
                if (data && data.getAllCollections) {
                    ...
                    if (preCreateLen > 0)
                        touchGetAllSanityFromCount(preCreateLen);
                }
                if (preCreateLen === 0) {
                ...
                if (
                    isSuspiciousEmptyAfterRecentNonEmptyList(preCreateLen) &&
                    preCreateLen === 0
                ) {
                    if (DEBUG_COLLECTIONS) {
                        try {
                            const h = getSharedDeduplicationWindow();
                            dlogPathB("refuse_create_flaky_getall_empty", {
                                pathB: pathBWindowSnapshot(),
                                s: h[GETALL_COLLECTIONS_SANITY] || null,
                            });
                        } catch (_) {
                            dlogPathB("refuse_create_flaky_getall_empty", {
                                pathB: pathBWindowSnapshot(),
            ...
            if (DEBUG_COLLECTIONS)
                dlogPathB("ensureBody_about_to_create", {
                    pathB: pathBWindowSnapshot(),
                });
            ...
            try {
                ...
                const recentAttemptAge =
                    getRecentPluginBackendCreateAttemptAgeMs(data);
                if (
                    recentAttemptAge != null &&
                    recentAttemptAge >= 0 &&
                    recentAttemptAge < 120000
                ) {
                ...
                const exactN =
                    await countExactPluginBackendNamedCollections(data);
                if (exactN >= 1) {
                    if (DEBUG_COLLECTIONS) {
                        dlogPathB("abort_create_exact_backend_name_exists", {
                            exactN,
                            ws: workspaceSlugFromData(data),
                        });
                    }
                ...
                const coll = await queueDataCreateOnSharedWindow(() =>
    ...
    function runPluginBackendEnsureWithLocksOrChain(data) {
        try {
            if (
                ...
                if (DEBUG_COLLECTIONS)
                    dlogPathB("ensure_route", {
                        via: "locks",
                        lockName: PB_LOCK_NAME,
                        pathB: pathBWindowSnapshot(),
                    });
                return navigator.locks.request(PB_LOCK_NAME, () =>
                    runPluginBackendEnsureBody(data),
                );
            }
        } catch (e) {
            if (DEBUG_COLLECTIONS)
                dlogPathB("ensure_locks_threw", {
                    err: String((e && e.message) || e),
                });
        }
        if (DEBUG_COLLECTIONS)
        ...
        return chainPluginBackendEnsure(data, () =>
            runPluginBackendEnsureBody(data),
        );
    }
    ...
    function ensurePluginSettingsCollection(data) {
        if (
            !data ||
            typeof data.getAllCollections !== "function" ||
            typeof data.createCollection !== "function"
        ) {
        ...
        if (DEBUG_COLLECTIONS) {
                ...
                dHint = data
                    ? `ctor=${(data && data.constructor && data.constructor.name) || "?"},eqPrev=${(data && data === g.__th_lastDataPb) || false},keys=${
                          Object.keys(data).filter(
                              (k) =>
                                  k &&
                                  (k.includes("thymer") || k.includes("__")),
                          ).length
                      }`
                    : "null";
            ...
            dlogPathB("ensurePluginSettingsCollection", {
                dataHint: dHint,
                dataExpand: (() => {
                    try {
                        if (!data) return { ok: false };
                        return { hasDataEnsure: !!data[DATA_ENSURE_P] };
                    } catch (_) {
                        return { ok: "throw" };
                    }
                })(),
        ...
        try {
            if (
                !data[DATA_ENSURE_P] ||
                typeof data[DATA_ENSURE_P].then !== "function"
            ) {
                data[DATA_ENSURE_P] = Promise.resolve();
            }
            if (DEBUG_COLLECTIONS)
                dlogPathB("data_ensure_p_chained", { hasPriorTail: true });
            const next = data[DATA_ENSURE_P].catch(() => {}).then(() =>
                runPluginBackendEnsureWithLocksOrChain(data),
            );
            data[DATA_ENSURE_P] = next;
            return next;
        } catch (e) {
            if (DEBUG_COLLECTIONS)
                dlogPathB("data_ensure_p_throw", {
                    err: String((e && e.message) || e),
                });
            ...
            if (guid) {
                for (let i = 0; i < 30; i++) {
                    await new Promise((res) =>
                        setTimeout(res, i < 8 ? 100 : 200),
                    );
                    try {
                        const again = await coll.getAllRecords();
                        r =
                            again.find((x) => x.guid === guid) ||
                            findVaultRecord(again, pluginId);
                        if (r) break;
                    } catch (_) {}
                }
            }
    ...
    function localMirrorMetaKey(pluginId) {
        return (
            LOCAL_MIRROR_META_PREFIX +
            encodeURIComponent(String(pluginId || "unknown"))
        );
    }
    ...
    function localPayloadMatchesRemote(keys, remote) {
        ...
        for (const k of keys) {
            ...
            if (localValue === null && typeof remoteValue !== "string")
                continue;
    ...
    function applyRemoteMirrorPayload(pluginId, keys, remote) {
        ...
        for (const k of keys) {
            ...
            console.warn(
                "[ThymerPluginSettings] Kept local settings instead of overwriting with older/ambiguous synced payload",
                {
                    pluginId,
                    key: k,
    ...
    async function listRows(data, { pluginSlug, recordKind } = {}) {
        ...
        return records.filter((r) => {
            ...
            if (recordKind != null && String(recordKind) !== "") {
                const rk =
                    rowField(r, FIELD_KIND) ||
                    inferRecordKindFromPid(pid, slug);
    ...
    async function createDataRow(
        data,
        { pluginSlug, recordKind, rowPluginId, recordTitle, settingsDoc } = {},
    ) {
        ...
        if (!ps || !rid || !kind) {
            console.warn(
                "[ThymerPluginSettings] createDataRow: pluginSlug, recordKind, and rowPluginId are required",
            );
            return null;
        }
        if (rid === ps && kind !== KIND_VAULT) {
            console.warn(
                "[ThymerPluginSettings] createDataRow: rowPluginId must differ from plugin slug unless record_kind is vault",
            );
        }
        await ensurePluginSettingsCollection(data);
        const coll = await findColl(data);
        ...
        try {
            ...
            console.error(
                "[ThymerPluginSettings] createDataRow createRecord",
                e,
            );
            return null;
        }
        ...
        for (let i = 0; i < 30; i++) {
            ...
            try {
                ...
                r =
                    again.find((x) => x.guid === guid) ||
                    again.find((x) => rowField(x, "plugin_id") === rid);
                if (r) break;
            } catch (_) {}
        }
        if (!r) return null;
    ...
    function showFirstRunDialog(ui, label, preferred, onPick) {
        ...
        card.style.cssText =
            "max-width:420px;width:100%;background:var(--panel-bg-color,var(--bg-default,Canvas));border:1px solid var(--border-default,currentColor);border-radius:12px;padding:20px;box-shadow:0 8px 32px rgba(0,0,0,0.5);";
        ...
        hint.textContent =
            "Change later via Command Palette → “Storage location…”";
        hint.style.cssText =
            "font-size:12px;color:var(--text-muted,currentColor);margin-bottom:16px;line-height:1.45;";
        const mk = (t, sub, prim) => {
            const b = document.createElement("button");
            b.type = "button";
            b.style.cssText =
                "display:block;width:100%;text-align:left;padding:12px 14px;margin-bottom:10px;border-radius:8px;cursor:pointer;font-size:14px;border:1px solid var(--border-default,currentColor);background:" +
                (prim ? "var(--bg-selected,var(--bg-hover,transparent))" : "transparent") +
                ";color:inherit;";
            ...
            if (sub) {
                ...
                s.textContent = sub;
                s.style.cssText =
                    "font-size:11px;opacity:0.75;margin-top:4px;line-height:1.35;";
                b.appendChild(s);
            }
            return b;
        };
        ...
        const bSyn = mk(
            ...
            "Store in the workspace “" +
                COL_NAME +
                "” collection (same account on any browser).",
            preferred === "synced",
        );
        const fin = (m) => {
            try {
    ...
    g.ThymerPluginSettings = {
        ...
        async init(opts) {
            ...
            const { plugin, pluginId, modeKey, mirrorKeys, label, data, ui } =
                opts;
            ...
            if (
                !mode &&
                remote &&
                (remote.storageMode === "synced" ||
                    remote.storageMode === "local")
            ) {
            ...
            plugin._pluginSettingsSyncMode =
                mode === "synced" ? "synced" : "local";
            plugin._pluginSettingsPluginId = pluginId;
            const keys =
                typeof mirrorKeys === "function" ? mirrorKeys() : mirrorKeys;
            let initFlushNeeded = false;

            if (
                plugin._pluginSettingsSyncMode === "synced" &&
                remote &&
                remote.payload &&
                typeof remote.payload === "object"
            ) {
                const applyResult = applyRemoteMirrorPayload(
                    pluginId,
                    keys,
                    remote,
                );
                initFlushNeeded = shouldFlushMirrorOnInit(
                    keys,
                    remote,
                    applyResult,
                );
        ...
        scheduleFlush(plugin, mirrorKeys) {
            ...
            if (plugin._pluginSettingsFlushTimer)
                clearTimeout(plugin._pluginSettingsFlushTimer);
            plugin._pluginSettingsFlushTimer = setTimeout(() => {
                ...
                if (!pid || !pdata) return;
                g.ThymerPluginSettings.flushNow(pdata, pid, keys).catch((e) =>
                    console.error("[ThymerPluginSettings] flush", e),
                );
            }, 500);
        },
        ...
        async openStorageDialog(opts) {
            ...
            const pick = await new Promise((resolve) => {
                ...
                const card = document.createElement("div");
                card.style.cssText =
                    "max-width:400px;width:100%;background:var(--panel-bg-color,var(--bg-default,Canvas));border:1px solid var(--border-default,currentColor);border-radius:12px;padding:18px;";
                ...
                [b1, b2].forEach((b) => {
                    b.style.cssText =
                        "display:block;width:100%;padding:10px 12px;margin-bottom:8px;border-radius:8px;cursor:pointer;border:1px solid var(--border-default,currentColor);background:transparent;color:inherit;text-align:left;";
                });
                ...
                bx.style.cssText =
                    "margin-top:8px;padding:8px 14px;border-radius:8px;cursor:pointer;border:1px solid var(--border-default,currentColor);background:transparent;color:inherit;";
    ...
    g.thymerExtInstallMobileInteractionGrace =
        installMobileInteractionGraceListener;
...
class Plugin extends AppPlugin {
    onLoad() {
        (async () => {
            await (globalThis.ThymerPluginSettings?.init?.({
                ...
                (console.warn(
                    "[Today's Notes] ThymerPluginSettings runtime missing (redeploy full plugin .js from repo).",
                ),
                Promise.resolve()));
            ...
            this.ui.registerCustomPanelType("tn-settings", (panel) =>
                this._mountSettingsPanel(panel),
            );
            this.ui.addCommandPaletteCommand({
                label: "Today's Notes: Settings",
                icon: "ti-settings",
                onSelected: () => this._openSettings(),
            });
            this.ui.addCommandPaletteCommand({
                label: "Today's Notes: Storage location…",
                icon: "ti-database",
                onSelected: () => {
            ...
            this._eventHandlerIds.push(
                this.events.on("panel.navigated", (ev) =>
                    setTimeout(() => this._handlePanel(ev.panel), 400),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("panel.focused", (ev) =>
                    this._handlePanel(ev.panel),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("panel.closed", (ev) =>
                    this._disposePanel(ev.panel?.getId?.()),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("record.created", () =>
                    this._onWorkspaceDataChanged(),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("record.updated", (ev) =>
                    this._onRecordUpdatedForNotes(ev),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("record.moved", () =>
                    this._onWorkspaceDataChanged(),
                ),
            );
    ...
    onUnload() {
        if (this._workspaceDataDebounceTimer) {
            try {
                clearTimeout(this._workspaceDataDebounceTimer);
            } catch (_) {}
        ...
        for (const id of Array.from((this._panelStates || new Map()).keys()))
            this._disposePanel(id, { permanent: true });
    ...
    _normalizeTimeMachine(tm) {
        ...
        const filters =
            Array.isArray(tm.filters) && tm.filters.length
                ? tm.filters.map((f, i) => ({
                ...
                : base.filters;
        const excludeJournalYearForMonthDay =
            tm.excludeJournalYearForMonthDay !== false;
        const g = String(tm.groupWithinYear || "")
            .trim()
            .toLowerCase();
    ...
    _loadSettings() {
        try {
            ...
            if (raw) {
                ...
                const msm = String(parsed.mainSortMode || "")
                    .trim()
                    .toLowerCase();
                return {
                    panelLabel:
                        typeof parsed.panelLabel === "string"
                            ? parsed.panelLabel
                            : "",
                    dateFields: Array.isArray(parsed.dateFields)
                        ? parsed.dateFields
                        : ["When", "when"],
                    excludedCollections: Array.isArray(
                        parsed.excludedCollections,
                    )
                        ? parsed.excludedCollections
                        : [],
                    mainSortMode: msm === "chrono" ? "chrono" : "collection",
                    chronoShowCollectionIcon:
                        parsed.chronoShowCollectionIcon !== false,
                    chronoShowCollectionName:
                        parsed.chronoShowCollectionName !== false,
    ...
    _saveSettings() {
        ...
        globalThis.ThymerPluginSettings?.scheduleFlush?.(this, () => [
            TN_SETTINGS_KEY,
            "tn_footer_collapsed",
        ]);
    }
    ...
    _settingsCacheSignature() {
        ...
        const fields = Array.isArray(this._settings?.dateFields)
            ? this._settings.dateFields
            : [];
        const excluded = Array.isArray(this._settings?.excludedCollections)
            ? this._settings.excludedCollections
            : [];
        const f = fields
            .map((x) =>
                String(x || "")
    ...
    _mainSortMode() {
        return this._settings?.mainSortMode === "chrono"
            ? "chrono"
            : "collection";
    }
        ...
        const next =
            mode === "chrono"
                ...
                : mode === "collection"
                  ? "collection"
                  : "";
        if (!next || !this._settings || this._settings.mainSortMode === next)
            return;
    ...
    _ensureDefaultActiveMain(state) {
        ...
        try {
            delete state._sectionCollapsed?.["main:chrono"];
        } catch (_) {}
        state.activeMain =
            this._mainSortMode() === "chrono" ? "chrono" : "collection";
    }
    ...
    _triMainAndTmAllOff(state) {
        ...
        const tmOff =
            !this._timeMachineEnabled() ||
            !!state._sectionCollapsed?.[TN_TM_SECTION_KEY];
    ...
    _resortMainResultsForPresentation(results, presentation) {
        ...
        if (presentation === "chrono") {
            arr.sort((a, b) => {
                const ta =
                    a.dateVal instanceof Date &&
                    !Number.isNaN(a.dateVal.getTime())
                        ? a.dateVal.getTime()
                        : 0;
                const tb =
                    b.dateVal instanceof Date &&
                    !Number.isNaN(b.dateVal.getTime())
                        ? b.dateVal.getTime()
                        : 0;
                if (ta !== tb) return ta - tb;
                return String(a.collectionName || "").localeCompare(
    ...
    _chronoRowMeta() {
        return {
            showCollectionIcon:
                this._settings?.chronoShowCollectionIcon !== false,
            showCollectionName:
                this._settings?.chronoShowCollectionName !== false,
        };
    }
    ...
    _applyFooterCollapsedVisual(root) {
        if (
            !root?.classList?.contains?.("tn-footer") ||
            root.classList.contains("tn-footer--suite-embed")
        )
            return;
    ...
    async _openSettings() {
        ...
        if (!panel) return;
        const type = this._tnJournalFooterSuiteEmbed
            ? "jfs-settings"
            : "tn-settings";
    ...
    async _appendTodaysNotesSettingsInto(containerEl, opts = {}) {
        ...
        const s = {
            ...
            chronoShowCollectionName:
                this._settings.chronoShowCollectionName !== false,
            timeMachine: JSON.parse(
                JSON.stringify(
                    this._normalizeTimeMachine(this._settings.timeMachine),
                ),
            ),
        };
        ...
        const render = () => {
            ...
            plInp.style.cssText =
                "width:100%;padding:7px 10px;border-radius:6px;font-size:13px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);outline:none;box-sizing:border-box;";
            plInp.addEventListener("input", () => {
                s.panelLabel = plInp.value;
            });
            ...
            s.dateFields.forEach((field, i) => {
                ...
                inp.value = field;
                inp.style.cssText =
                    "flex:1;padding:7px 10px;border-radius:6px;font-size:13px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);outline:none;";
                inp.addEventListener("input", () => {
                    s.dateFields[i] = inp.value.trim();
                });
                const rm = document.createElement("button");
                rm.textContent = "✕";
                rm.style.cssText =
                    "background:none;border:none;color:var(--text-muted,currentColor);cursor:pointer;font-size:13px;padding:4px 6px;flex-shrink:0;";
                rm.addEventListener("click", () => {
                    s.dateFields.splice(i, 1);
                    render();
                });
                row.appendChild(inp);
                row.appendChild(rm);
            ...
            addFieldBtn.style.cssText =
                "padding:6px 12px;background:transparent;border:1px dashed var(--border-default,currentColor);border-radius:6px;font-size:12px;color:var(--text-muted,currentColor);cursor:pointer;margin-top:4px;";
            ...
            collections.forEach((name) => {
                const row = document.createElement("div");
                row.style.cssText =
                    "display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid var(--divider-color,var(--border-subtle,currentColor));";
                const cb = document.createElement("input");
                cb.type = "checkbox";
                cb.checked = !s.excludedCollections.has(name);
                cb.style.cssText =
                    "width:15px;height:15px;cursor:pointer;accent-color:var(--accent,var(--link-color,currentColor));flex-shrink:0;";
            ...
            sortLab.style.cssText =
                "display:block;font-size:12px;color:var(--text-muted,currentColor);margin-bottom:6px;";
            sortLab.textContent = "Order notes";
            dispSec.appendChild(sortLab);

            const sortSel = document.createElement("select");
            sortSel.style.cssText =
                "width:100%;padding:7px 10px;border-radius:6px;font-size:13px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);margin-bottom:10px;";
            [
                ["collection", "By collection (default)"],
                ["chrono", "By time (chronological within the day)"],
            ].forEach(([val, lab]) => {
            ...
            sortSel.value =
                s.mainSortMode === "chrono" ? "chrono" : "collection";
            sortSel.addEventListener("change", () => {
                s.mainSortMode =
                    sortSel.value === "chrono" ? "chrono" : "collection";
                render();
            });
            dispSec.appendChild(sortSel);

            const chronoOpts = document.createElement("div");
            chronoOpts.style.cssText =
                "display:flex;flex-direction:column;gap:8px;padding:10px 0 4px;border-top:1px solid var(--divider-color,var(--border-subtle,currentColor));";
            ...
            const mkChronoCb = (label, key) => {
                ...
                cb.style.cssText =
                    "width:15px;height:15px;cursor:pointer;accent-color:var(--accent,var(--link-color,currentColor));";
                cb.addEventListener("change", () => {
                    s[key] = cb.checked;
                });
            ...
            chronoOpts.appendChild(
                mkChronoCb("Show collection icon", "chronoShowCollectionIcon"),
            );
            chronoOpts.appendChild(
                mkChronoCb("Show collection name", "chronoShowCollectionName"),
            );
            ...
            tmEn.style.cssText =
                "display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;margin-bottom:10px;";
            const tmCb = document.createElement("input");
            tmCb.type = "checkbox";
            tmCb.checked = s.timeMachine.enabled !== false;
            tmCb.addEventListener("change", () => {
                s.timeMachine.enabled = tmCb.checked;
            });
            ...
            s.timeMachine.filters = Array.isArray(s.timeMachine.filters)
                ? s.timeMachine.filters
                : [];
            const tmFiltersWrap = document.createElement("div");
            tmFiltersWrap.style.cssText =
                "display:flex;flex-direction:column;gap:8px;margin-top:12px;border-top:1px solid var(--divider-color,var(--border-subtle,currentColor));padding-top:12px;";

            const opChoices = [
                ["same_day_last_year", "Same calendar day, last year"],
                ["on_journal_day", "On journal day"],
                [
                    "same_month_day_as_journal",
                    "Same month/day as journal (any year)",
                ],
            ...
            const renderTmFilters = () => {
                ...
                s.timeMachine.filters.forEach((rule, ridx) => {
                    ...
                    fin.style.cssText =
                        "flex:1;min-width:100px;padding:6px 8px;border-radius:6px;font-size:12px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);";
                    fin.addEventListener("input", () => {
                        s.timeMachine.filters[ridx].field = fin.value.trim();
                    });
                    const opSel = document.createElement("select");
                    opSel.style.cssText =
                        "padding:6px 8px;border-radius:6px;font-size:12px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);";
                    ...
                    opSel.value = opChoices.some(([v]) => v === rule.op)
                        ? rule.op
                        : "same_day_last_year";
                    opSel.addEventListener("change", () => {
                        s.timeMachine.filters[ridx].op = opSel.value;
                    });
                    ...
                    vin.style.cssText =
                        "flex:1;min-width:80px;padding:6px 8px;border-radius:6px;font-size:12px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);";
                    vin.addEventListener("input", () => {
                        s.timeMachine.filters[ridx].value = vin.value;
                    });
                    const rm = document.createElement("button");
                    rm.type = "button";
                    rm.textContent = "✕";
                    rm.style.cssText =
                        "background:none;border:none;color:var(--text-muted,currentColor);cursor:pointer;font-size:12px;padding:4px;";
                    rm.addEventListener("click", () => {
                        s.timeMachine.filters.splice(ridx, 1);
                        renderTmFilters();
                    });
                    row.appendChild(fin);
            ...
            addTmFilter.style.cssText =
                "margin-top:6px;padding:6px 12px;background:transparent;border:1px dashed var(--border-default,currentColor);border-radius:6px;font-size:12px;color:var(--text-muted,currentColor);cursor:pointer;";
            addTmFilter.addEventListener("click", () => {
                s.timeMachine.filters.push({
                    id: `tm_${Date.now()}`,
                    field: "When",
                    op: "same_day_last_year",
                    value: "",
            ...
            tmExtra.style.cssText =
                "display:flex;flex-direction:column;gap:10px;margin-top:18px;padding-top:16px;border-top:1px solid var(--divider-color,var(--border-subtle,currentColor));";
            tmExtra.appendChild(
                this._cfgLabel(
                    "Same month/day as journal",
            ...
            exclLb.style.cssText =
                "display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;";
            const exclCb = document.createElement("input");
            exclCb.type = "checkbox";
            exclCb.checked =
                s.timeMachine.excludeJournalYearForMonthDay !== false;
            exclCb.style.cssText =
                "width:15px;height:15px;cursor:pointer;accent-color:var(--accent,var(--link-color,currentColor));";
            exclCb.addEventListener("change", () => {
                s.timeMachine.excludeJournalYearForMonthDay = exclCb.checked;
            });
            const exclSp = document.createElement("span");
            exclSp.textContent =
                'Exclude journal calendar year from "same month/day" results';
            ...
            tmGroupSel.style.cssText =
                "width:100%;padding:7px 10px;border-radius:6px;font-size:13px;background:var(--bg-default,Canvas);color:inherit;border:1px solid var(--border-default,currentColor);";
            [
            ...
            tmGroupSel.value =
                s.timeMachine.groupWithinYear === "chrono"
                    ? "chrono"
                    : "collection";
            tmGroupSel.addEventListener("change", () => {
                s.timeMachine.groupWithinYear =
                    tmGroupSel.value === "chrono" ? "chrono" : "collection";
            });
            ...
            if (!suiteEmbed) {
                ...
                saveBtn.style.cssText =
                    "padding:10px 0;background:var(--accent,var(--link-color,currentColor));color:var(--accent-fg,var(--bg-default,Canvas));border:none;border-radius:8px;font-weight:700;font-size:14px;cursor:pointer;width:100%;";
                saveBtn.addEventListener("click", () => {
                    applyDraft();
                    this.ui.addToaster({
                        title: "Saved",
                        message: "Today's Notes settings saved.",
                        dismissible: true,
    ...
    async _mountSettingsPanel(panel) {
        ...
        el.innerHTML = "";
        el.style.cssText =
            "padding:0;overflow:auto;height:100%;box-sizing:border-box;";
    ...
    _handlePanel(panel) {
        ...
        if (!container) {
            ...
            if (!state) {
                ...
                state._containerWatcher = new MutationObserver(() => {
                    ...
                    if (newContainer) {
                        try {
                            state._containerWatcher?.disconnect();
                        } catch (_) {}
                ...
                try {
                    state._containerWatcher.observe(panelEl, {
                        childList: true,
                        subtree: true,
                    });
                } catch (_) {
        ...
        if (!state) {
            ...
            if (state.timeMachineResults === undefined)
                state.timeMachineResults = null;
            if (state.timeMachineLoading === undefined)
                state.timeMachineLoading = false;
            // Reset state when date changes
            if (dateChanged) {
                if (
                    this._tnJournalFooterSuiteEmbed &&
                    this._tnPreservedSectionCollapse
                ) {
    ...
    _disposePanel(panelId, opts = {}) {
        ...
        if (opts.permanent && this._tnPreservedSectionCollapse) {
            ...
            for (const k of Array.from(
                this._tnPreservedSectionCollapse.keys(),
            )) {
                if (k.startsWith(prefix))
                    this._tnPreservedSectionCollapse.delete(k);
            }
        } else if (this._tnJournalFooterSuiteEmbed && s) {
            try {
                if (!this._tnPreservedSectionCollapse)
                    this._tnPreservedSectionCollapse = new Map();
                const key = `${panelId}\t${String(s.journalDate || "")}`;
                this._tnPreservedSectionCollapse.set(
                    key,
                    JSON.parse(JSON.stringify(s._sectionCollapsed || {})),
                );
            } catch (_) {}
        }
        ...
        try {
            s._containerWatcher?.disconnect();
        } catch (_) {}
    ...
    _mountFooter(state, container, panelEl) {
        ...
        const stale = container.querySelectorAll(
            `:scope > .tn-footer[data-panel-id="${state.panelId}"]`,
        );
    ...
    _timeMachineEnabled() {
        ...
        return (
            !!tm.enabled && Array.isArray(tm.filters) && tm.filters.length > 0
        );
    }

    _collectionSectionKey(name) {
        return `coll:${String(name || "")
            .trim()
            .toLowerCase()}`;
    }
    ...
    _createTriHeaderRow(state) {
        ...
        menuBtn.type = "button";
        menuBtn.className =
            "tn-tri-menu button-none button-small button-minimal-hover";
        ...
        tmBtn.type = "button";
        tmBtn.className =
            "tn-tri-tm button-none button-small button-minimal-hover";
    ...
    _syncTriHeaderRow(state, tnRoot, footCollapsed) {
        const row =
            ...
            tnRoot
                ?.closest?.(".jfs-shell")
                ?.querySelector?.(".jfs-tn-extras .tn-header-tri");
        if (!row) return;
        if (footCollapsed) {
        ...
        if (menuBtn) {
            ...
            menuBtn.title = on
                ? "By collection (click to hide)"
                : "By collection (click to show)";
        }
        if (clockBtn) {
            ...
            clockBtn.title = on
                ? "By time (click to hide)"
                : "By time (click to show)";
        }
        if (tmBtn) {
            if (!this._timeMachineEnabled()) {
                tmBtn.disabled = true;
                ...
                tmBtn.title = on
                    ? "Time Machine (click to hide)"
                    : "Time Machine (click to show)";
            }
        }
    }
    ...
    _resolveJfsTnExtrasHost(state, tnRoot) {
        ...
        try {
            ...
            if (
                ...
                shell = panelEl.querySelector(
                    `.jfs-shell[data-panel-id="${CSS.escape(id)}"]`,
                );
            }
        } catch (_) {}
    ...
    _syncTnHeaderExtras(state) {
        ...
        if (!tnRoot) return;
        const footCollapsed = this._tnJournalFooterSuiteEmbed
            ? false
            : this._collapsed;
        ...
        const suiteDom = !!tnRoot.classList?.contains?.(
            "tn-footer--suite-embed",
        );
        ...
        for (const collName of byColl.keys()) {
            ...
            btn.type = "button";
            btn.className =
                "tn-collapsed-chip button-none button-small button-minimal-hover";
            ...
            const rawIcon =
                (state._lastCollectionIcons &&
                    state._lastCollectionIcons[collName]) ||
                "";
    ...
    _tmItemYear(item) {
        const d =
            item.dateVal instanceof Date &&
            !Number.isNaN(item.dateVal.getTime())
                ? item.dateVal
    ...
    _appendCollectionIconVisual(parent, rawIcon) {
        ...
        const slug = (
            s.startsWith("ti-") ? s.slice(3) : s.replace(/^ti-?/i, "")
        )
            .replace(/_/g, "-")
            .replace(/\s+/g, "-");
    ...
    _collectionIconName(coll) {
        ...
        for (const raw of candidates) {
            ...
            if (/^[a-z0-9_-]+$/i.test(raw))
                return `ti-${raw.replace(/^ti-?/, "").replace(/_/g, "-")}`;
    ...
    _evaluateTmFilterRule(record, rule, journalParts, dayRange, lastYearRange) {
        ...
        const raw = isDateOp
            ? this._readDateFieldNamed(record, field)
            : this._readFieldValueSimple(record, field);
        const value = (v) => {
            if (v == null) return "";
        ...
        if (op === "same_month_day_as_journal") {
            ...
            if (
                ...
                return false;
            const tmCfg = this._normalizeTimeMachine(
                this._settings?.timeMachine,
            );
            if (
                tmCfg.excludeJournalYearForMonthDay !== false &&
    ...
    async _queryTimeMachineRecords(journalYyyymmdd) {
        const excludedSet = new Set(
            (this._settings.excludedCollections || []).map((n) =>
                n.toLowerCase(),
            ),
        );
        const journalNames = new Set(["journal", "journals"]);
        ...
        for (const coll of collections) {
            ...
            const tm0 = this._normalizeTimeMachine(this._settings.timeMachine)
                .filters[0];
            for (const record of records) {
                if (!this._recordPassesTmFilters(record, journalYyyymmdd))
                    continue;
    ...
    async _runTimeMachineGenerate(state) {
        ...
        this._syncTnHeaderExtras(state);
        try {
            state.timeMachineResults = await this._queryTimeMachineRecords(
                state.journalDate,
            );
        } catch (e) {
    ...
    _renderFooterBody(state) {
        ...
        const results =
            !raw.length || pres === "main_collapsed"
                ? raw
                : this._resortMainResultsForPresentation(
                      raw.map((x) => ({ ...x })),
                      pres,
                  );
        bodyEl.innerHTML = "";
        ...
        if (!raw.length) {
            if (!this._timeMachineEnabled()) {
                bodyEl.innerHTML =
                    '<div class="tn-empty">No notes found for this day.</div>';
            }
        } else {
            if (pres === "main_collapsed") {
                ...
                for (const item of results)
                    list.appendChild(this._buildRow(item, state, rowMeta));
                ...
                for (const [collName, items] of byColl) {
                    ...
                    const icon =
                        ...
                        "";
                    bodyEl.appendChild(
                        this._buildCollectionSectionEl(
                            collName,
                            items,
                            icon,
                            sk,
        ...
        const hasCollapsed = Object.keys(state._sectionCollapsed || {}).some(
            (k) => state._sectionCollapsed[k],
        );
    ...
    _buildTimeMachineSectionEl(state) {
        ...
        const rowMeta = this._chronoRowMeta();
        const { yearMap, years, unknown } =
            this._groupTmResultsByYearDescending(state.timeMachineResults);
        ...
        const renderYearBucket = (yearLabel, items) => {
            ...
            const collNames = Array.from(byColl.keys()).sort((a, b) =>
            ...
            for (const collName of collNames) {
                ...
                for (const item of this._sortTmItemsByTimeAscending(
                    byColl.get(collName) || [],
                )) {
        ...
        for (const y of years)
            renderYearBucket(String(y), yearMap.get(y) || []);
    ...
    _buildRoot(state) {
        ...
        root.className = this._tnJournalFooterSuiteEmbed
        ...
        calToggle.className =
            "tn-calendar-toggle button-none button-small button-minimal-hover";
        calToggle.title = this._collapsed
            ? "Show today's notes"
            : "Hide today's notes";
        ...
        settingsBtn.className =
            "tn-settings-btn button-none button-small button-minimal-hover";
    ...
    async _openRecord(panel, recordGuid, lineGuid, e) {
        ...
        if (this._wantsSidePanel(e)) {
            try {
                ...
                await this._navigatePanelToRecord(
                    newPanel,
                    recordGuid,
                    null,
                    workspaceGuid,
    ...
    _buildPanelNavActions(recordGuid, lineGuid = "") {
        ...
        here.type = "button";
        here.className =
            "tn-panel-nav-btn button-none button-small button-minimal-hover tooltip";
    ...
    _handleClick(e, state) {
        ...
        if (action === "expand-record") {
            ...
            if (groupEl)
                this._toggleRecordExpansion(state, recordGuid, groupEl).catch(
                    () => {},
                );
        }
        ...
        if (action === "toggle-preview-node") {
            ...
            if (cached?.collapsedNodes) {
                ...
                if (cached.collapsedNodes.has(nodeGuid))
                    cached.collapsedNodes.delete(nodeGuid);
    ...
    async _populate(state) {
        ...
        try {
            const onScanProgress = (msg) => {
                if (
                    state.rootEl !== targetRootEl ||
                    state.journalDate !== targetDate
                )
                    return;
                const el = targetRootEl?.querySelector(".tn-loading");
                if (el) el.textContent = msg;
            };
            let results = await this._getRecordsForDate(
                targetDate,
                onScanProgress,
            );
            ...
            results = this._resortMainResultsForPresentation(
                results,
                this._effectiveMainPresentation(state),
            );
            ...
            if (state.rootEl === targetRootEl && targetRootEl.isConnected) {
                bodyEl.innerHTML =
                    '<div class="tn-empty">Error loading records.</div>';
            }
        }
    ...
    _buildExpandRecordBtn(recordGuid, state) {
        const isExpanded =
            state?.recordExpandedState?.get?.(recordGuid)?.expanded === true;
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className =
            "tlr-expand-record-btn button-none" +
            (isExpanded ? " is-expanded" : "");
        btn.dataset.action = "expand-record";
        btn.dataset.recordGuid = recordGuid;
        btn.title = isExpanded
            ? "Hide record preview"
            : "Preview record content inline";
    ...
    async _toggleRecordExpansion(state, recordGuid, groupEl) {
        ...
        const expandBtn = groupEl.querySelector(
            `.tlr-expand-record-btn[data-record-guid="${recordGuid}"]`,
        );
        const previewEl = groupEl.querySelector(".tlr-record-preview");
        const cached = state.recordExpandedState.get(recordGuid);

        if (cached?.expanded) {
            state.recordExpandedState.set(recordGuid, {
                expanded: false,
                allItems: null,
        ...
        if (previewEl) {
            ...
            l.className = "tlr-expand-loading";
            l.textContent = "Loading…";
            previewEl.appendChild(l);
        }
        ...
        try {
            ...
            const transclusionItems = rawItems.filter(
                (item) => item?.type === "ref" || item?.type === "transclusion",
            );
            if (transclusionItems.length > 0) {
                console.log("[TN-TRANSCLUSION-DEBUG]", {
                    recordGuid,
                    count: transclusionItems.length,
                    items: transclusionItems,
                });
            }

            const allItems = await this._resolveTransclusions(
                rawItems,
                recordGuid,
            );
            ...
            if (previewEl) {
                ...
                e.className = "tlr-expand-empty";
                e.textContent = "Could not load record content.";
    ...
    async _resolveTransclusions(items, recordGuid) {
        ...
        for (const item of items) {
            const isTransclusion =
                item?.type === "ref" || item?.type === "transclusion";
            if (!isTransclusion) {
                result.push(item);
                continue;
            }
            ...
            try {
                ...
                const refItems = refRecord
                    ? await refRecord.getLineItems()
                    : null;
                if (refItems && refItems.length > 0) {
                    const transParent = item.parent_guid || recordGuid;
                    const mapped = refItems.map((ri) => {
                        const isRoot =
                            !ri.parent_guid || ri.parent_guid === refGuid;
                        return isRoot
                            ? Object.assign({}, ri, {
                                  parent_guid: transParent,
                              })
                            : ri;
                    });
                    result.push(...mapped);
    ...
    _renderRecordPreview(
        ...
        const renderNode = (item, depth) => {
            ...
            if (hasChildren) {
                ...
                childrenEl.className =
                    "tlr-preview-children" + (isCollapsed ? " is-hidden" : "");
        ...
        for (const root of childrenOf.get(recordGuid) || [])
    ...
    _appendLineText(container, line) {
        ...
        if (line?.type === "ref" || line?.type === "transclusion") {
            const refGuid =
                line?.ref_guid ||
                line?.guid_ref ||
                line?.segments?.[0]?.text?.guid;
            const name = refGuid
                ? this._resolveRecordName(refGuid) || "[transcluded block]"
                : "[transcluded block]";
    ...
    _appendSegments(container, segments) {
        for (const seg of segments || []) {
            ...
            if (seg.type === "text") {
                ...
                el.textContent =
                    seg.text?.title ||
                    this._resolveRecordName(seg.text?.guid) ||
                    "[link]";
                container.appendChild(el);
                ...
                a.className = "tn-seg-link";
                a.textContent = seg.text?.title || url;
    ...
    _resolveRecordName(guid) {
        try {
            return this.data.getRecord?.(guid)?.getName?.() || null;
        } catch (_) {
            return null;
        }
    }
    ...
    async _getRecordsForDate(yyyymmdd, onProgress) {
        ...
        if (
            hit &&
            now - hit.ts < TN_QUERY_CACHE_TTL_MS &&
        ...
        const excludedSet = new Set(
            this._settings.excludedCollections.map((n) => n.toLowerCase()),
        );
        const journalNames = new Set(["journal", "journals"]);
        const collections = await this.data.getAllCollections();
        ...
        for (let i = 0; i < toScan.length; i++) {
            ...
            if (onProgress) {
                try {
                    onProgress(
                        total
                            ? `Scanning collections… ${i + 1}/${total}`
        ...
        try {
            this._recordsByDateCache.set(cacheKey, { ts: now, results });
        } catch (_) {}
        return results;
    }
    ...
    _getDateFieldValue(record) {
        const fields =
            this._settings.dateFields.length > 0
                ? this._settings.dateFields
                : ["When", "when"];
    ...
    _cfgLabel(title, subtitle) {
        ...
        t.textContent = title;
        t.style.cssText =
            "font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:0.07em;color:var(--text-muted,currentColor);margin-bottom:4px;";
        wrap.appendChild(t);
        if (subtitle) {
            const s = document.createElement("div");
    ...
    _injectCSS() {
      ...
      .tn-footer {
        ...
        border: 1px solid var(--divider-color, var(--border-subtle, currentColor));
        border-radius: 10px;
      ...
      .tn-footer:not(.tn-footer--suite-embed).tn-footer--collapsed {
        ...
        border-color: var(--divider-color, var(--border-subtle, currentColor));
        color: inherit;
      ...
      .jfs-tn-extras .tn-header-tri-divider {
        ...
        height: 14px;
        margin: 0 5px 0 5px;
        background: var(--divider-color, var(--border-subtle, currentColor));
        opacity: 0.35;
        flex-shrink: 0;
        align-self: center;
      }
      ...
      .tn-footer .tn-section {
        margin-top: 10px;
        border-top: 1px solid var(--divider-color, var(--border-subtle, currentColor));
        padding-top: 8px;
      }
      ...
      .tn-footer .tn-section-collapse-btn:hover { color: inherit; }
      .tn-footer .tn-section-body { padding-left: 2px; }
      .tn-footer .tn-tm-section { border-top: 1px solid var(--divider-color, var(--border-subtle, currentColor)); }
      ...
      .tn-footer .tn-tm-year-head {
        ...
        margin: 12px 0 6px;
        padding-bottom: 4px;
        border-bottom: 1px solid var(--divider-color, var(--border-subtle, currentColor));
      }
      ...
      .tn-footer .tn-header .tn-settings-btn:hover { color: inherit; }
      ...
      .tn-footer .tn-record-group:not(.tlr-record-expanded) .tn-row:hover { background: var(--bg-hover, transparent); }
      .tn-footer .tn-record-group.tlr-record-expanded .tn-row { background: var(--bg-hover, transparent); border-radius: 6px 6px 0 0; }
      ...
      .tn-footer .tn-record-name:hover { color: inherit; }
      ...
      .tn-footer .tlr-expand-record-btn:hover { color: inherit; }
      .tn-footer .tlr-expand-record-btn.is-expanded { color: var(--ed-link-color,var(--link-color,var(--accent,inherit))); }

      .tn-footer .tlr-record-preview {
        display: none;
        flex-direction: column;
        margin: 0 0 6px 10px;
        border-left: 2px solid var(--divider-color, var(--border-subtle, currentColor));
      ...
      .tn-footer .tlr-preview-toggle:hover { color: inherit; }
      ...
      .tn-footer .tlr-expand-line:hover { background: var(--bg-hover, transparent); color: inherit; }

      .tn-footer .tn-prefix { color: var(--text-muted, currentColor); font-size: 11px; flex-shrink: 0; margin-right: 2px; }
      .tn-footer .tn-line-content strong { color: inherit; }
      .tn-footer .tn-line-content em { opacity: 0.8; }
      .tn-footer .tn-line-content code { font-family: monospace; font-size: 11px; background: var(--bg-hover, transparent); padding: 0 3px; border-radius: 3px; }
      .tn-footer .tn-seg-ref  { color: var(--ed-link-color,var(--link-color,var(--accent,inherit))); }
      .tn-footer .tn-seg-link { color: var(--ed-link-color,var(--link-color,var(--accent,inherit))); text-decoration: none; }
